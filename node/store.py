...
class MemoryStore:
    ...
    def _id(self, value: dict[str, Any]) -> str:
        for key in ("id", "album_id", "media_id", "source_id", "relationship_id", "link_id"):
            if isinstance(value.get(key), str) and value[key].startswith("moh:"):
                return value[key]
        if "record_id" in value and value["record_id"].startswith("moh:"):
    ...
    def get_all(self, collection: str) -> list[dict[str, Any]]:
        rows = self.db.execute("SELECT payload FROM objects WHERE collection=? ORDER BY object_id", (collection,)).fetchall()
        return [json.loads(row["payload"]) for row in rows]

    def get(self, collection: str, object_id: str) -> dict[str, Any] | None:
        row = self.db.execute("SELECT payload FROM objects WHERE collection=? AND object_id=?", (collection, object_id)).fetchone()
        return json.loads(row["payload"]) if row else None

    def upsert(self, collection: str, value: dict[str, Any]) -> None:
        object_id = self._id(value)
        self.db.execute(
            "INSERT INTO objects(collection, object_id, payload) VALUES(?,?,?) "
            "ON CONFLICT(collection, object_id) DO UPDATE SET payload=excluded.payload",
            (collection, object_id, json.dumps(value, sort_keys=True, ensure_ascii=False)),
        )
        self.db.commit()

    def remove(self, collection: str, object_id: str) -> None:
        self.db.execute("DELETE FROM objects WHERE collection=? AND object_id=?", (collection, object_id))
        self.db.commit()

    def add_change(self, envelope: dict[str, Any]) -> int:
        cur = self.db.execute("INSERT INTO changes(envelope) VALUES(?)", (json.dumps(envelope, sort_keys=True, ensure_ascii=False),))
        self.db.commit()
        return int(cur.lastrowid)

    def changes_after(self, cursor: int, limit: int = 100) -> list[dict[str, Any]]:
        rows = self.db.execute("SELECT cursor, envelope FROM changes WHERE cursor>? ORDER BY cursor LIMIT ?", (cursor, limit)).fetchall()
        return [{"cursor": str(row["cursor"]), "envelope": json.loads(row["envelope"])} for row in rows]

    def current_cursor(self) -> str:
        row = self.db.execute("SELECT COALESCE(MAX(cursor), 0) AS cursor FROM changes").fetchone()
